...
bool FLAGS_dauntless = false;
bool FLAGS_override_keyid = false;
						...
						map<string, uint32_t>* bits)
{
	...
	for (cur_node = a_node; !done && cur_node; cur_node = cur_node->next) {
		...
		if (cur_node->type == XML_TEXT_NODE && (content = xmlNodeGetContent(cur_node)) != NULL) {
...
#else
	FATAL("code signer is not built with JSON support!\n");
	result = true;
#endif // HAVE_JSON
	return result;
}
...
void usage(int argc, char* argv[])
{
	fprintf(stderr,
		...
		"[--override-keyid] to force the JSON specified keyid\n"
		"[--dauntless] to sign a Dauntless image\n"
		"[--verbose]\n",
		argv[0]);
}
...
int getOptions(int argc, char *argv[])
{
	static struct option long_options[] = {
		...
		{"override-keyid", no_argument, NULL, 'K'},
		{"dauntless", no_argument, NULL, 'd'},
		{"writefuses", required_argument, NULL, 'w'},
		{"dev_id0", required_argument, NULL, '0'},
		{"dev_id1", required_argument, NULL, '1'},
		{0, 0, 0, 0}
	};
...
int main(int argc, char* argv[])
{
	...
	// Fill the image if requested.
	if (fillPattern) image.fillPattern(pattern);
	if (fillRandom) image.fillRandom();

	// Let's start preparinga header.
	SignedHeader hdr;
	hdr.keyid = key.n0inv(); // montgomery inverse of key's n0
	hdr.ro_base = image.ro_base();
	hdr.ro_max = image.ro_max();
	hdr.rx_base = image.rx_base();
	hdr.rx_max = image.rx_max() + 12;  // TODO: m3 instruction prefetch sets off GLOBALSEC when too tight make sure these are nops or such?
	...
	if (!jsonFilename.empty() && !readJSON(jsonFilename, &tag, &values, &fuses, &infos))
		FATAL("Failed to read JSON from '%s', either not found or invalid!\n", jsonFilename.c_str());
	...
	if (values["board_id"] && values["board_id_flags"]) {
		// Set the board ID if the manifest has it.
		hdr.board_id_type = values["board_id"];
		hdr.board_id_type_mask = values["board_id_mask"];
		hdr.board_id_flags = values["board_id_flags"];
	}else{
		// If no board ID was specified, set all of its bits to 0.
		hdr.board_id_type = hdr.board_id_type_mask = hdr.board_id_flags = 0;
	}

	// Store the board ID XOR of SIGNED_HEADER_PADDING since that's how older images were signed.
	hdr.board_id_type ^= SIGNED_HEADER_PADDING;
	hdr.board_id_type_mask ^= SIGNED_HEADER_PADDING;
	hdr.board_id_flags ^= SIGNED_HEADER_PADDING;

	VERBOSE("timestamp: %ld\n", hdr.timestamp_);

	// Make sure the keyid matches the value in the manifest.
	if (values["keyid"] != hdr.keyid && !FLAGS_override_keyid)
		FATAL("mismatched keyid JSON %d vs. key %d\n", values["keyid"], hdr.keyid);
	else if (FLAGS_override_keyid)
		hdr.keyid = values["keyid"];
	...
	if (!xmlFilename.empty() && !readXML(xmlFilename, &fuse_ids, &fuse_bits, &xml_p4cl))
		FATAL("Failed to read XML from '%s', either not found or invalid!\n", xmlFilename.c_str());
